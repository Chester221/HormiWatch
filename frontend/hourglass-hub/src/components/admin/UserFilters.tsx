...
import {
  SlidersHorizontal, Check, ChevronDown, Users, Shield, Briefcase, Wrench,
  CheckCircle, Ban, X,
} from "lucide-react";
...
const STATUS_OPTIONS: UserFilterOption[] = [
...
export const DEFAULT_ROLE_OPTIONS: UserFilterOption[] = [
  { value: "all", label: "Todos", icon: Users, color: "text-muted-foreground" },
...
export function UserFilters({
  ...
  resultCount, activeCount, onClear,
}: UserFiltersProps) {
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="end" className="w-[300px] rounded-2xl border-border/60 bg-card p-0 shadow-2xl">
        {/* Cabecera */}
        <div className="flex items-center gap-3 border-b border-border/40 px-4 py-3.5">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">Filtrar usuarios</p>
            <p className="text-[10px] text-muted-foreground">
              {resultCount} resultado{resultCount !== 1 ? "s" : ""} de usuarios
            </p>
          </div>
        </div>
