  ...
  useEffect(() => {
    const fetchProjectEvents = async () => {
      ...
      try {
        ...
        const projects = Array.isArray(responseProjects) ? responseProjects : responseProjects?.data || [];
        
        const responseTasks = await tasksApi.getAll();
        const tasks = Array.isArray(responseTasks) ? responseTasks : responseTasks?.data || [];
