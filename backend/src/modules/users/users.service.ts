...
export class UsersService {
  ...
  // findTechnicianRole() es tolerante a mayúsculas/acentos y crea el rol si
  // no existe, garantizando que el registro siempre tenga un rol válido.
  async registerPublic(email: string, password: string, name: string) {
    const role = await this.roleService.findTechnicianRole();
    ...
    try {
      ...
      return await this.userRepository.manager.transaction(async (tx) => {
        ...
        const profileData = {
          ...
          full_name: `${userData.name} ${userData.lastName || ''}`.trim(),
        };
  ...
  async update(
    ...
    // ✅ CORREGIDO: Actualizar rol por nombre si se envía 'role' como string
    if (userData.role) {
      try {
        const roleEntity = await this.roleService.findOneByName(userData.role);
        if (roleEntity) {
          role = roleEntity;
        } else {
          this.logger.warn(`Role with name '${userData.role}' not found`);
        }
      } catch (error) {
        this.logger.warn(`Error finding role by name '${userData.role}': ${error.message}`);
      }
    }
    ...
    try {
      await this.userRepository.manager.transaction(async (manager) => {
        ...
        // ✅ AGREGAR avatar_url y full_name al perfil
        user.profile.avatar_url = userData.avatar_url ?? user.profile.avatar_url;
        user.profile.full_name = userData.full_name ?? user.profile.full_name;
